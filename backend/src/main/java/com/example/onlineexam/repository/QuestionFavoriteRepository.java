...
public interface QuestionFavoriteRepository extends JpaRepository<QuestionFavorite, Long> {
    @Query("select qf from QuestionFavorite qf where qf.userId = :userId and qf.question.deleted = false order by qf.createTime desc")
    List<QuestionFavorite> findByUserIdOrderByCreateTimeDesc(@Param("userId") Long userId);

    @Query("select qf from QuestionFavorite qf where qf.userId = :userId and qf.question.id = :questionId and qf.question.deleted = false")
    List<QuestionFavorite> findByUserIdAndQuestionId(@Param("userId") Long userId, @Param("questionId") Long questionId);
